...
<body>
    ...
    // Verificar si se envió el formulario de editar producto
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['accion']) && $_POST['accion'] == 'editar') {
        $id_producto = $_POST['id_producto'];

        // Consulta SQL para obtener los datos del producto seleccionado
        $sql_obtener_producto = "SELECT * FROM productos WHERE id = '$id_producto'";
        $result_producto = mysqli_query($conexion, $sql_obtener_producto);

        if ($result_producto && mysqli_num_rows($result_producto) > 0) {
            $row_producto = mysqli_fetch_assoc($result_producto);

            // Mostrar el formulario para editar los atributos del producto
    ?>
            <h1>Editar Producto</h1>
            <form action="actualizar_producto.php" method="post">
                <input type="hidden" name="id_producto" value="<?php echo $row_producto['id']; ?>">
                <label for="nombre">Nombre:</label>
                <input type="text" id="nombre" name="nombre" value="<?php echo $row_producto['nombre']; ?>" required><br><br>

                <label for="descripcion">Descripción:</label>
                <input type="text" id="descripcion" name="descripcion" value="<?php echo $row_producto['descripcion']; ?>" required><br><br>

                <label for="precio">Precio:</label>
                <input type="number" id="precio" name="precio" value="<?php echo $row_producto['precio']; ?>" required min="1"><br><br>

                <label for="precio_reventa">Precio Reventa:</label>
                <input type="number" id="precio_reventa" name="precio_reventa" value="<?php echo $row_producto['precio_reventa']; ?>" required min="1"><br><br>

                <label for="stock">Stock:</label>
                <input type="number" id="stock" name="stock" value="<?php echo $row_producto['stock']; ?>" required min="1"><br>
